    ...
    <div class="screen screen-wide">
        ...
        <div style="display:flex; align-items:center; gap:10px;">
          <div class="pts-pill-wrap">
            <div class="pts-pill ${gainToShow ? 'pts-pulse' : ''}" style="display:flex; align-items:center; gap:8px; height:34px; padding:0 14px; border-radius:17px; background:#15130F;">
              <span style="font-size:11px; color:#A6842C;">&#9670;</span>
              <span style="font-size:12.5px; font-weight:500; letter-spacing:.10em; color:#F3EEE3;">${p.points} pts</span>
            </div>
      ...
      <div style="padding:0 22px 14px;">
        <div style="display:flex; justify-content:space-between; align-items:baseline; margin-bottom:7px;">
          <span style="font-size:11px; font-weight:400; letter-spacing:.10em; color:#4A443A;">${tier.prevEarned ? tier.prevEarned.name + ' · next ' + tier.next.name : 'Next: ' + tier.next.name}</span>
          <span style="font-size:11px; font-weight:300; color:#8C8375;">${tier.atTop ? 'Top tier reached' : Math.round(tier.next.at - p.points) + ' pts to go'}</span>
        </div>
      ...
      <div style="padding:14px 22px 0;">
        <p style="margin:0 0 4px; text-align:center; font-size:12px; font-weight:300; color:#8C8375;">You've voted on <span style="font-weight:500; color:#1B1916;">${votedCount}</span> of <span style="font-weight:500; color:#1B1916;">${totalAccepted}</span> available images.</p>
        ${!celebrating ? `<p style="margin:0 0 12px; text-align:center;"><span id="jump-points-btn" style="font-size:11px; font-weight:300; color:#8C8375; text-decoration:underline; cursor:pointer;">Jump to 25 pts (testing)</span></p>` : ''}
        ${bottomNav('vote')}
      </div>
    </div>
  ...
  return `
    <div id="vote-card" class="vote-card" style="border-radius:20px; overflow:hidden; background:#FFFFFF; box-shadow:0 18px 40px rgba(27,25,22,.13);">
      <div class="vote-photo-wrap" style="position:relative;">
        <div class="vote-photo" style="width:100%; aspect-ratio:1.42; ${photoStyle(img.photo)} display:block;"></div>
        <div style="position:absolute; left:14px; top:14px; height:28px; padding:0 12px; border-radius:14px; background:rgba(27,25,22,.62); backdrop-filter:blur(6px); display:flex; align-items:center; font-size:10.5px; letter-spacing:.16em; text-transform:uppercase; color:#F3EEE3;">${remaining} left</div>
      </div>
      <div style="padding:18px 20px 20px;">
        <p class="h-serif" style="font-size:21px; line-height:1.25; margin-bottom:4px;">${esc(img.title)}</p>
        <p style="margin:0 0 12px; font-size:12px; font-weight:300; letter-spacing:.06em; color:#8C8375;">by ${esc(img.credit || 'a student')}</p>
        <p style="margin:0; font-size:13.5px; font-weight:300; line-height:1.65; color:#4A443A;">${esc(img.description)}</p>
      </div>
    </div>
    <div style="display:flex; align-items:center; justify-content:center; gap:30px; margin-top:20px;">
      <button id="pass-btn" style="width:66px; height:66px; border-radius:33px; background:#FFFFFF; border:1px solid rgba(196,84,58,.35); color:#C4543A; display:flex; align-items:center; justify-content:center; font-size:13px; font-weight:500; letter-spacing:.10em; text-transform:uppercase; cursor:pointer; box-shadow:0 6px 16px rgba(27,25,22,.08);">Pass</button>
      <button id="note-btn" style="height:44px; padding:0 18px; border-radius:22px; border:1px solid rgba(27,25,22,.18); background:#fff; display:flex; align-items:center; font-size:11px; font-weight:500; letter-spacing:.12em; text-transform:uppercase; color:#4A443A; cursor:pointer;">Note +3</button>
      <button id="like-btn" style="width:66px; height:66px; border-radius:33px; background:#2E6B5C; color:#FFFDF8; border:none; display:flex; align-items:center; justify-content:center; font-size:13px; font-weight:500; letter-spacing:.10em; text-transform:uppercase; cursor:pointer; box-shadow:0 8px 20px rgba(46,107,92,.30);">Like</button>
    </div>
    <p class="swipe-hint" style="margin:14px 0 0; text-align:center; font-size:12px; font-weight:300; color:#8C8375;">Swipe right to like, left to pass</p>
  `;
}
...
function celebrationHTML(p) {
  ...
  return `
    <div style="position:relative; height:100%; display:flex; flex-direction:column; align-items:center; justify-content:center; text-align:center; padding:40px 0 20px;">
      ...
      <p style="margin:0 0 10px; font-size:10.5px; letter-spacing:.24em; text-transform:uppercase; color:#A6842C;">${earned ? earned.at : ''} points reached</p>
      ...
      <div style="width:100%; max-width:280px; border-radius:18px; background:#15130F; padding:22px; margin-bottom:24px;">
        <p style="margin:0 0 6px; font-size:10.5px; letter-spacing:.20em; text-transform:uppercase; color:#9A8F79;">Your prize</p>
        <p class="h-serif" style="margin:0 0 8px; font-size:28px; color:#A6842C;">${copy.prize}</p>
        <p style="margin:0; font-size:13px; font-weight:300; line-height:1.6; color:#CFC7B6;">${copy.detail}</p>
      </div>

      <p style="margin:0 0 26px; max-width:260px; font-size:14px; font-weight:300; line-height:1.7; color:#5B5449;">${earned ? earned.perk + '.' : ''}</p>
...
function finishedHTML() {
  return `
    <div style="height:100%; display:flex; flex-direction:column; align-items:center; justify-content:center; text-align:center; padding:40px 0 20px;">
      ...
      <p style="margin:0 0 22px; max-width:270px; font-size:14px; font-weight:300; line-height:1.7; color:#5B5449;">You've voted on all currently available images. Check back as new images are added.</p>
      <span id="reset-votes-btn" style="font-size:11.5px; letter-spacing:.10em; text-transform:uppercase; color:#8C8375; cursor:pointer; text-decoration:underline;">Reset my votes (testing)</span>
    </div>`;
}
